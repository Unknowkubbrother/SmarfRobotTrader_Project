...
if [[ -z "${VISION_LLM_EMBED_TEXT_API_URL:-}" ]]; then
  if [[ "$VISION_LLM_API_URL" == */ ]]; then
    export VISION_LLM_EMBED_TEXT_API_URL="${VISION_LLM_API_URL}embed_text"
  else
    export VISION_LLM_EMBED_TEXT_API_URL="${VISION_LLM_API_URL}/embed_text"
  fi
fi
...
if [[ -z "${LIVE_MANAGED_MAGIC_SET:-}" ]]; then
  ...
  export LIVE_MANAGED_MAGIC_SET="${BASE_MANAGED_MAGIC},123456,12345"
  if is_truthy "${LIVE_MANAGE_MANUAL_POSITIONS:-0}"; then
    export LIVE_MANAGED_MAGIC_SET="${LIVE_MANAGED_MAGIC_SET},0"
  fi
fi
...
export BOT_RUNTIME_VERSION_STATE_FILE="$INSTANCE_DIR/runner_version_state.env"
...
echo "VISION_LLM_EMBED_TEXT_API_URL: ${VISION_LLM_EMBED_TEXT_API_URL:-auto}"
...
echo "LIVE_CATCHUP_MAX_BARS: ${LIVE_CATCHUP_MAX_BARS:-auto}"
echo "LIVE_SEMANTIC_ALIAS_HOURS: ${LIVE_SEMANTIC_ALIAS_HOURS:-auto}"
echo "LIVE_SEMANTIC_NO_DATA_RETRY_SECONDS: ${LIVE_SEMANTIC_NO_DATA_RETRY_SECONDS:-auto}"
...
echo "BOT_RUNTIME_VERSION_STATE_FILE: ${BOT_RUNTIME_VERSION_STATE_FILE:-auto}"
