...
export default function Auth() {
  ...
  const { user, signIn, signInWithGoogle, registerOTP, verifyOTP, completeRegistration } = useAuth();
  ...
  const step1Form = useForm<z.infer<typeof registerStep1Schema>>({
    resolver: zodResolver(registerStep1Schema),
    defaultValues: { email: "", password: "", confirmPassword: "" },
  });

  const step2Form = useForm<z.infer<typeof registerStep2Schema>>({
    resolver: zodResolver(registerStep2Schema),
    defaultValues: { recoveryEmail: "" },
  });

  const step4Form = useForm<z.infer<typeof registerStep4Schema>>({
  ...
  const handleStep1 = async (data: z.infer<typeof registerStep1Schema>) => {
    setRegData(prev => ({ ...prev, email: data.email, password: data.password }));
    setStep(2);
  };

  const handleStep2 = async (data: z.infer<typeof registerStep2Schema>) => {
  ...
  const handleStep4 = async (data: z.infer<typeof registerStep4Schema>) => {
    setIsLoading(true);
    try {
            ...
            {mode === "login" ? (
              <>
                ...
                <p className="mt-6 text-center text-sm text-muted-foreground">
                  ...
                  <button onClick={() => { setMode("register"); setStep(1); }} className="text-foreground font-medium hover:underline">Sign Up</button>
                </p>
              </>
            ) : (
              <>
                {step === 1 && (
                  <>
                    ...
                    <Button variant="outline" className="w-full h-11 rounded-full mb-4 gap-2" onClick={handleGoogleLogin} disabled={isLoading}>
                      <svg className="w-5 h-5" viewBox="0 0 24 24">
                        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                        <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                ...
                <p className="mt-6 text-center text-sm text-muted-foreground">
                  ...
                  <button onClick={() => { setMode("login"); setStep(1); }} className="text-foreground font-medium hover:underline">Sign In</button>
                </p>
              </>
            )}
          </div>
