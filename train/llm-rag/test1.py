...
def visualize_debug(image_path: str, lo_p=20, hi_p=95, heat_alpha=0.55, dim_factor=0.25):
    ...
    H, W = img_np.shape[:2]
    heat = np.kron(sim_grid, np.ones((H // g, W // g)))[:H, :W]
    keep_masked = np.kron(sim_grid > 0.5, np.ones((H // g, W // g)))[:H, :W]
    masked = img_np.copy()
    masked[keep_masked < 0.5] *= float(dim_factor)
    ...
    plt.subplot(1, 3, 3); plt.title("Masked"); plt.imshow(masked); plt.axis("off")
    plt.tight_layout()
    plt.show()
    ...
    # for ly in layers:
    #     hs = vision_out.hidden_states[ly]   # [1, 257, D]
    #     patch_tokens = hs[:, 1:, :]         # [1, 256, D]
    #     patch_emb = clip_model.visual_projection(patch_tokens)  # [1, 256, E]
    #     patch_emb = F.normalize(patch_emb, dim=-1)

    #     sim_anchor = (patch_emb @ anchor_feats.T).max(dim=-1).values.squeeze(0)  # [256]
    #     sim_candle = (patch_emb @ candle_feats.T).max(dim=-1).values.squeeze(0)  # [256]
    #     sim = sim_candle + 0.15 * sim_anchor
    #     sims.append(sim)

    # # รวมหลาย layer (mean หรือ weighted)
    # sim = torch.stack(sims, dim=0).mean(dim=0)  # [256]
    # sim_np = sim.detach().cpu().numpy()
    ...
    # plt.subplot(1, 2, 1); plt.title("Wide (ROI+letterbox)"); plt.imshow(img_np); plt.axis("off")
    # plt.subplot(1, 2, 2); plt.title(f"Heatmap MAX-prompt (layer {ly})"); plt.imshow(img_np); plt.imshow(heat, alpha=float(heat_alpha)); plt.axis("off")
    # plt.tight_layout()
    # plt.show()
...
if __name__ == "__main__":
    ...
    QUERY_IMAGE = "datasets1/new_chart1.png"
